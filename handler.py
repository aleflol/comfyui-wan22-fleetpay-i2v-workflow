...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            # Handle video/gif outputs (VHS_VideoCombine etc.)
            for media_key in ("gifs", "videos"):
                if media_key in node_output:
                    ...
                    for media_info in node_output[media_key]:
                        ...
                        if os.environ.get("BUCKET_ENDPOINT_URL"):
                            try:
                                with tempfile.NamedTemporaryFile(suffix=file_extension, delete=False) as tf:
                                    tf.write(media_bytes)
                                    tpath = tf.name
                                s3_url = rp_upload.upload_image(job_id, tpath)
                                os.remove(tpath)
                            ...
                            try:
                                ...
                                print(f"worker-comfyui - Encoded {filename} as base64 ({len(media_bytes)} bytes)")
                            except Exception as e:
